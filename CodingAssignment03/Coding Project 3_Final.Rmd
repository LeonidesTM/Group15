...
```
## Answer
We are running a logarithm on BMI and Age since those are the things that would intuitively put more stress on the healthcare system and result in more frequent visits to a PCP

Running a log on BMI results in a more normally distributed set of values per histogram. Running a natural log on Age did not seem to do much for the distribution. Went from fairly normal to more normal. 
